...
from models.quantization.quantization_modules import u_quant_weight_conv, u_quant_weight_linear, u_quant_weight_conv1d
...
spike_q_param_timewise = False

T_max = 3

qk_pos= False #v pos and neg
...
#new version
# from models.quantization.quantization_modules_new import spike_quant_fea
...
class SpikeConv2d_q(nn.Conv2d):
    def __init__(
        ...
        self.layer_type = 'conv2d' if 'special_layer' not in kwargs or kwargs['special_layer'] is None else kwargs['special_layer']
        if fea_all_positive:
        ...
        self.spike_q_param_timewise = spike_q_param_timewise
        if spike_q_param_timewise:
            ...
            self.spikes_quant = spike_quant_fea(self.spike_quant_channel, self.spike_bit, gama_init=gama_init,gama_std=gama_std, learn_spike_bit=self.learn_spike_bit, all_positive=self.fea_all_positive)
    ...
    def forward(self, x, q_info=None):
        ...
        if self.training and self.init_training_state == 0:
            ...
            if self.spike_quant_channel is None: # per layer
                if self.fea_all_positive:
                    ...
                    if self.spike_q_param_timewise:
                        for i in range(len(self.spikes_quant)):
                            torch.nn.init.constant_(self.spikes_quant[i].gama, 2 * x[i].detach().abs().mean() / math.sqrt(2**(self.spike_bit)-1))
                    else:
                        torch.nn.init.constant_(self.spikes_quant.gama, 2 * x.detach().abs().mean() / math.sqrt(2**(self.spike_bit)-1))
                            
                else:
                    
                    if self.spike_q_param_timewise:
                        for i in range(len(self.spikes_quant)):
                            torch.nn.init.constant_(self.spikes_quant[i].gama, 2 * x[i].detach().abs().mean() / math.sqrt(2**(spike_init_bit-1)-1))
                    else:
                        torch.nn.init.constant_(self.spikes_quant.gama, 2 * x.detach().abs().mean() / math.sqrt(2**(spike_init_bit-1)-1))
                            
            else:
                if self.fea_all_positive:
                    if self.spike_q_param_timewise:
                        for i in range(len(self.spikes_quant)):
                            self.spikes_quant[i].gama.data.copy_(2 * x[i].detach().abs().mean(dim=(0,2,3), keepdim=True) / math.sqrt(2**(self.spike_bit)-1))
                    else:
                        self.spikes_quant.gama.data.copy_(2 * x.detach().abs().mean(dim=(0,1,3,4), keepdim=True) / math.sqrt(2**(self.spike_bit)-1))
                else:
                    if self.spike_q_param_timewise:
                        for i in range(len(self.spikes_quant)):
                            self.spikes_quant[i].gama.data.copy_(2 * x[i].detach().abs().mean(dim=(0,2,3), keepdim=True) / math.sqrt(2**(spike_init_bit-1)-1))
                    else:
                        self.spikes_quant.gama.data.copy_(2 * x.detach().abs().mean(dim=(0,1,3,4), keepdim=True) / math.sqrt(2**(spike_init_bit-1)-1))
        ...
        if self.spike_q_param_timewise:
            for i in range(t):
                mem = mem_res * self.decay +  x[i]
                output.append(self.spikes_quant[i](mem))
                mem_res = mem.detach() - output[i].detach()
        else:
            for i in range(t):
                mem = mem_res * self.decay +  x[i]
                output.append(self.spikes_quant(mem))
                mem_res = mem.detach() - output[i].detach()
        spike_out = torch.stack(output, dim=0)
        ...
        if q_info is not None:
            if self.spike_q_param_timewise:
                ...
                q_info.append({
                            'spike_bit': torch.stack([ste_clamp(self.spikes_quant.bit.clone().abs(), min=1., max=self.spikes_quant.bit_bound) for i in range(1)]).squeeze().reshape(-1),
                            'spike_scale': torch.stack([self.spikes_quant.gama.clone() for i in range(1)]).squeeze().reshape(-1),
                            'weight_bit': ste_clamp(self.weight_quant.bit.clone().abs().squeeze().reshape(-1), 1.,self.weight_quant.bit_bound),
                            'weight_scale': self.weight_quant.alh.clone().abs().squeeze().reshape(-1),
                            'T': T_grad.clone().reshape(-1),
                            'T_max':self.T_max,
        ...
        if not self.training and ace_cal:    
            B,C,H,W = x.shape
            spike_bit = q_info[-1]['spike_bit'].detach()
            weight_bit = q_info[-1]['weight_bit'].detach()
        ...
        out = self._conv_forward(spike_out.flatten(0,1), weight_q, self.bias)
        out_tb, out_c, out_h, out_w = out.shape
        return out.reshape(t, b, out_c, out_h, out_w), q_info
...
class SpikeLinear_q(nn.Linear):
    def __init__(
                ...
                fea_all_positive=False,init_weight_bit=weight_bit, step = 2, init_spike_bit=2, max_step=4, 
                para_dict={'alh_init':0.01,'alh_std':0.02,'gama_init':0.1,'gama_std':0.2},
                **kwargs
                ) -> None:
        super().__init__(in_features, out_features, bias)
        self.layer_type = 'linear' if 'special_layer' not in kwargs or kwargs['special_layer'] is None else kwargs['special_layer']
        if fea_all_positive:
            self.spike_bit = init_spike_bit
            print('fea_all_positive')
        else:
        ...
        if spike_q_param_timewise:
            ...
            self.spikes_quant = spike_quant_fea(None, self.spike_bit, gama_init=gama_init,gama_std=gama_std, learn_spike_bit=self.learn_spike_bit, all_positive=self.fea_all_positive)
    ...
    def forward(self, x, q_info=None):
        ...
        if not self.training and ace_cal:    
            ...
            # print('T:', valid_T)
        try:    
            out = F.linear(spike_out.flatten(0,1), weight_q, self.bias)
        except RuntimeError:
            print('debug=='*30)
            print(spike_out.dtype)
            print(weight_q.dtype)
            raise RuntimeError
        
        try:
            out_tb, out_n, out_c = out.shape
            return out.reshape(t,b,out_n,out_c), q_info
        except ValueError:
            out_tb, out_c = out.shape
            return out.reshape(t,b,out_c), q_info
...
class SpikeConv2d_full(nn.Conv2d):
    def __init__(
                self, in_channels: int, out_channels: int, kernel_size, stride = 1, padding = 0, dilation = 1, groups = 1, bias = True, padding_mode = 'zeros', device=None, dtype=None,
    ...
    def forward(self, x, q_info=None):
        # x: [t*b,c,h,w]
        weight_q = self.weight_quant(self.weight, self.weight_bit)
        return self._conv_forward(x, weight_q, self.bias), q_info

    
    @staticmethod
    def inherit_from(ori_module, step=2, **kwargs):
        assert isinstance(ori_module, torch.nn.Conv2d)
        new_module = SpikeConv2d_full(
            in_channels = ori_module.in_channels,
            out_channels = ori_module.out_channels,
            kernel_size = ori_module.kernel_size,
            stride = ori_module.stride,
...
class Dcls1d_q(_DclsNd):
    ...
    **kwargs) -> None:
        ...
        super().__init__(            
                ...
                padding_mode,
                version,)
        self.layer_type = 'dcls1d' if 'special_layer' not in kwargs or kwargs['special_layer'] is None else kwargs['special_layer']
        if fea_all_positive:
            self.spike_bit = init_spike_bit
            print('fea_all_positive')
        else:
        ...
        # weight quantization module
        self.weight_per_layer = kwargs['weight_per_layer']
        self.weight_quant = u_quant_weight_conv1d(out_channels, self.weight_bit, alh_init=alh_init,alh_std=alh_std, learn_weight_bit = self.learn_weight_bit, weight_per_layer=kwargs['weight_per_layer'])
        # self.weight_quant = nn.Identity()
        ...
        self.Lif = neuron.LIFNode(tau=10.05, v_threshold=1.0, 
    ...
    def forward(self, x, max_delay, q_info=None):
        #lsq initial
        if self.training and self.init_training_state == 0:
            print('initial scales for the first forward in training')
            weight_init_bit = self.weight_bit if self.weight_bit > 1 else 2
            ...
            # init wegith alpha
            if self.weight_per_layer:
                torch.nn.init.constant_(self.weight_quant.alh, 2 * self.DCK(self.weight.detach(), self.P.detach(), self.SIG.detach()).detach().abs().mean() / math.sqrt(2**(weight_init_bit-1)-1))
            else:
                self.weight_quant.alh.data.copy_(2 * self.DCK(self.weight.detach(), self.P.detach(), self.SIG.detach()).detach().abs().mean(dim=list(range(1, self.DCK(self.weight.detach(), self.P.detach(), self.SIG.detach()).dim())), keepdim=True) / math.sqrt(2**(weight_init_bit-1)-1))
        ...
        # weight quantization
        # weight_q = self.weight_quant(self.DCK(self.weight, self.P, self.SIG), self.weight_bit)
        weight_q = self.DCK(self.weight, self.P, self.SIG)

        # spike quantization expected fea
        # (batch, neurons, time) b,c,t 
        t,b,c = x.shape #actual fea
        
        #SNN quant
        output = []
        mem_res = 0.
        # if not self.layer_type=='first dcls1d':
        #     if self.spike_q_param_timewise:
        #         for i in range(t):
        #             mem = mem_res * self.decay +  x[i]
        #             output.append(self.spikes_quant[i](mem))
        ...
        # if self.spike_q_param_timewise:
        #     for i in range(t):
        #         output.append(self.spikes_quant[i](x[i]))
        # else:
        #     for i in range(t):
        #         output.append(self.spikes_quant(x[i]))
            
        # spike_out = torch.stack(output, dim=0)
        if self.layer_type=='first dcls1d':
            spike_out = x
        else:
            spike_out = self.Lif(x)
        # spike_out = x
        ...
        if not self.training and ace_cal:    
            ...
            stride = self.stride if not isinstance(self.stride, tuple) else self.stride[0]
            padding = self.padding if not isinstance(self.padding, tuple) else self.padding[0]
            ops = math.ceil((N + padding * 2 - (kernel_size-1)) / stride) ** 2 * kernel_size ** 2 * C * self.out_channels
            sops = ops * torch.min((valid_T*spike_bit.mean()), weight_bit.mean())
            s_ace = (bit_budget * ops).mean()
            f_num = 0
            for out in output:
            ...
            # self.DCK(self.weight.detach(), self.P.detach(), self.SIG.detach())
        #permute, pad, and conv
        spike_out = spike_out.permute(1,2,0) #spike:tbc->bct
        left_padding = max_delay-1
        right_padding = (max_delay-1) // 2
        spike_out = F.pad(spike_out, (left_padding, right_padding), 'constant', 0)    
        out = self._conv_forward(spike_out, weight_q, self.bias) 
        return out.permute(2,0,1), q_info #spike:bct->tbc

    @staticmethod
    def inherit_from(ori_module, step=2, **kwargs):
        assert isinstance(ori_module, Dcls1d)
        new_module = Dcls1d_q(
            in_channels = ori_module.in_channels,
            out_channels = ori_module.out_channels,
            kernel_count = ori_module.kernel_count,
            groups = ori_module.groups,
            version = ori_module.version,
            dilated_kernel_size = ori_module.dilated_kernel_size,
            bias = ori_module.bias is not None,
        ...
        new_module.weight.data.copy_(ori_module.weight.detach())
        new_module.P.data.copy_(ori_module.P.detach())
        new_module.P.requires_grad = ori_module.P.requires_grad
        new_module.SIG.data.copy_(ori_module.SIG.detach())
        new_module.SIG.requires_grad = ori_module.SIG.requires_grad
        if ori_module.bias is not None:
            new_module.bias.data.copy_(ori_module.bias.detach())
...
class SpikeLinear_full(nn.Linear):
    def __init__(
        ...
        self.spike_q_param_timewise = False
        if spike_q_param_timewise:
            self.spikes_quant = nn.ModuleList([
                spike_quant_fea(None, self.spike_bit, gama_init=gama_init,gama_std=gama_std, learn_spike_bit=self.learn_spike_bit, all_positive=self.fea_all_positive) for i in range(self.T_max)
            ])
