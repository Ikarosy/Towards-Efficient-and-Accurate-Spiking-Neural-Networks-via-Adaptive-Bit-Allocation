...
class basic_downsample(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, bias,) -> None:
        super().__init__()
        self.downsample = nn.Conv2d(in_channels=in_planes, out_channels=out_planes, kernel_size=kernel_size, stride=stride, bias=bias)
...
class SEW_downsample(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, bias,) -> None:
        super().__init__()
        self.downsample = nn.Conv2d(in_channels=in_planes, out_channels=out_planes, kernel_size=kernel_size, stride=stride, bias=bias)
        self.bn = BN(out_planes)
...
class ResNet_Cifar(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = basic_downsample(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False)
...
class ResNet_Cifar_real(nn.Module):

    def __init__(self, block, layers, input_c=3, num_classes=10, rp = False):
        super(ResNet_Cifar_real, self).__init__()
...
class SEWResNet_Cifar(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = SEW_downsample(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False)
...
def resnet20_cifar_real(**kwargs):
    model = ResNet_Cifar_real(BasicBlock, [3, 3, 3], **kwargs)
    return model
