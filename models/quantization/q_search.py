...
class ObserverBase(nn.Module):
    ...
    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
        ...
        if self.symmetric:
            self.quant_min = -2 ** (self.bit - 1) + 1 if self.bit > 1 else -1
            self.quant_max = 2 ** (self.bit - 1) - 1 if self.bit > 1 else 1
        else:
            self.quant_min = 0
    ...
    def calculate_qparams(self, min_val, max_val):
        ...
        if self.symmetric:
            ...
            scale = torch.max(scale, self.eps)
            zero_point = torch.round(min_val_neg / scale) - quant_min
...
class MSEObserver(ObserverBase):
    ...
    def forward(self, x_orig):
        ...
        if self.one_side_dist != 'no' or self.symmetric:  # one-side distribution or symmetric value for 1-d search
            ...
            raise NotImplemented('2D search should not occur')
